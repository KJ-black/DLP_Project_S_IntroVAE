...
def train_soft_intro_vae(dataset='cifar10', z_dim=128, lr_e=2e-4, lr_d=2e-4, batch_size=128, num_workers=4, start_epoch=0,
    ...
    if dataset == '102flowers':
        ...
        train_set = ImageDatasetFromFile(train_list, embeddings, data_root, input_height=None, crop_height=None,
    ...
    model = SoftIntroVAE(cdim=ch, zdim=z_dim, emb_dim=emb_dim, channels=channels, image_size=image_size, conditional=True).to(device)
    ...
    for epoch in tqdm.notebook.tqdm(range(start_epoch, num_epochs)):
        ...
        for iteration, batch in enumerate(train_data_loader, 0):
            ...
            # ELBO for real data
            real_mu, real_logvar = model.encode(real_batch, o_cond=emb)
            z = reparameterize(real_mu, real_logvar)
            rec = model.decoder(z, y_cond=emb)
            ...
            rec_mu, rec_logvar, z_rec, rec_rec = model(rec.detach(), o_cond=emb.detach())
            fake_mu, fake_logvar, z_fake, rec_fake = model(fake.detach(), o_cond=emb.detach())
            ...
            rec_rec = model.decode(z_rec.detach(), y_cond=emb.detach())
            rec_fake = model.decode(z_fake.detach(), y_cond=emb.detach())
            ...
            if cur_iter % test_iter == 0:
                ...
                _, _, _, rec_det = model(real_batch, o_cond=emb, deterministic=True)
