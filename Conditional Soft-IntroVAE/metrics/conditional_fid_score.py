...
def get_activations_generate(dataloader, model_s, model, batch_size=50, dims=2048,
                             cuda=False, verbose=False, device=torch.device("cpu"), num_images=50000):
    """Calculates the activations of the pool_3 layer for all images.
    Params:
    -- files       : List of image files paths
    ...
    for i, x in tqdm(enumerate(dataloader, 0)):
        label = x[1].to(device)
        batch_size = label.shape[0]
        # torch.cuda.set_device(0)
        noise_batch = torch.randn(size=(batch_size, model_s.zdim)).to(device)
        images = model_s.sample(noise_batch, y_cond=label)
...
def calculate_activation_statistics_generate(dataloader, model_s, model, batch_size=50,
                                             dims=2048, cuda=False, verbose=False, device=torch.device("cpu"),
                                             num_images=50000):
    """Calculation of the statistics used by the FID.
    Params:
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations_generate(dataloader, model_s, model, batch_size, dims, cuda, verbose, device, num_images)
    mu = np.mean(act, axis=0)
    sigma = np.cov(act, rowvar=False)
    return mu, sigma
...
def _compute_statistics_of_generate(dataloader, model_s, model, batch_size, dims, cuda, device, num_images):
    m, s = calculate_activation_statistics_generate(dataloader, model_s, model, batch_size,
                                                    dims, cuda, device=device, num_images=num_images)
...
def calculate_fid_given_dataset(dataloader, model_s, batch_size, cuda, dims, device, num_images):
    ...
    m2, s2 = _compute_statistics_of_generate(dataloader, model_s, model, batch_size,
                                             dims, cuda, device, num_images)
    fid_value = calculate_frechet_distance(m1, s1, m2, s2)
